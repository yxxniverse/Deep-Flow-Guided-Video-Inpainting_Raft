...
from models import RAFT_Models
...
def parse_args():
    ...
    parser.add_argument('--pretrained_model_raft', type=str,
                        default='./pretrained_models/raft-small')
...
def infer(args):
    ...
    Raft = RAFT_Models.raft(args, requires_grad=False)
    print('====> Loading', args.pretrained_model_raft)
    raft_ckpt = torch.load(args.pretrained_model_raft)
    Raft.load_state_dict(raft_ckpt['state_dict'])
    Raft.to(device)
    Raft.eval()
